...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static IConfigurationBuilder AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "Learn.Bit.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("Learn.Bit.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"Learn.Bit.Client.Core.appsettings.{AppEnvironment.Current}.json");
